...
/*   Updated: 2023/10/10 03:14:38 by yatsu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_file	*ft_init_file(char *path, int *error, int param)
{
	...
	file->fd = 0;
	if (param && ((access(file->path, F_OK)) || (!access(file->path, F_OK) && (!access(path, W_OK)))))
		return (file);
	else if (param)
		return (*error = 7, file);
	if (!file->path)
...
t_pipex	*ft_init_pipex(char **argv, char **env)
{
	...
	if (pipex->error)
		return (pipex);
	pipex->f2 = ft_init_file(argv[3], &(pipex->error), 1);
	return (pipex);
}
